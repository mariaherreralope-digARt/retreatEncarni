  ...
  return (
    <section id="retiros" className="relative mx-24 py-12">
      ...
      <div className="relative h-[350px] flex items-center group">
        {/* Navigation Buttons */}
        <button
          className="absolute md:-right-12 -right-14 top-1/2 transform -translate-y-1/2 z-20 bg-primary hover:bg-white hover:border hover:border-primary hover:text-primary text-white rounded-full p-2 transition duration-300"
          onClick={() => swiperRef.current?.slideNext()}
        >
          <BsChevronRight className="w-6 h-6" />
        </button>
        <button
          className="absolute md:-left-12 -left-14 top-1/2 transform -translate-y-1/2 z-20 bg-primary hover:bg-white hover:border hover:border-primary hover:text-primary text-white rounded-full p-2 transition duration-300"
        ...
        <div className="absolute inset-0 bg-slate-700/40 z-10 flex flex-col items-center justify-center text-center px-4">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="max-w-3xl"
            >
              <h2 className="text-slate-100 font-heading  mt-2 md:mt-0text-lg  md:text-3xl font-bold mb-4">
                {slides[activeIndex].title}
              </h2>
              <p className="text-slate-300 text-md font-body md:text-xl">
